...
const credentials = {
  ...
  redirect_uris: ["https://hientrancong.github.io/meet/"],
  javascript_origins: ["https://hientrancong.github.io", "http://localhost:3000"],
};
...
module.exports.getCalendarEvents = async (event) => {
  ...
  //Set access_token as credentials in oAuth2Client
  oAuth2Client.setCredentials({ access_token });

  return new Promise((resolve, reject) => {
    //calendar method to get a list of events using oAuth2Client for authentication
    calendar.events.list(
      ...
      //use a callback to get error and response to use for promise
      (error, response) => {
        if (error) {
          reject(error);
        } else {
